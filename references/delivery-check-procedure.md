...
**Workaround:** Deliver via Telegram (see above). Note: OAuth credentials may still be valid even when the package is missing — check `~/.google_workspace_mcp/credentials/` for token freshness before assuming auth is also broken.
